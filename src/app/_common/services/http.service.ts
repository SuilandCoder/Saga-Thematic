...
export class HttpService {
    ...
    geojsonToShape(geojson: any, srsStr: string): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            this.http.post('/api/json2shp', fd).toPromise().then(res => {
                resolve(res);
            }).catch(error => {
                console.log(error);
            })
    ...
    get_SGRD_ColorMap(layerItem:LayerItem):Promise<Object>{
        return new Promise((resolve,reject)=>{
            ...
            if(layerItem.uploaded && layerItem.dataId !==null){
                ...
                this.http.post("/api/getRemoteSgrdInformation", postData).toPromise().then(next => {
                    resolve(next);
                }).catch(error => {
                    reject(error);
                })
                ...
                this.http.post("/api/getSgrdInformation", postData).toPromise().then(next => {
                    resolve(next);
                }).catch(error => {
                    reject(error);
                })
    ...
    getColorMap(layerItem: LayerItem, colorBand: string): Promise<Object> {
        return new Promise((resolve, reject) => {
            ...
            if (layerItem.uploaded && layerItem.dataId !== null) {
                ...
                this.http.post("/api/getRemoteTifInformation", postData).toPromise().then(next => {
                    // let RsltJson = next;
                ...
                this.http.post("/api/getTifInformation", postData).toPromise().then(next => {
                    resolve(next);
                }).catch(error => {
                    reject(error);
                })
